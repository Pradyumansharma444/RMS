...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-background text-foreground overflow-hidden">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed md:static inset-y-0 left-0 z-50 w-64 bg-sidebar border-r border-sidebar-border flex flex-col transform ${
      ...
      >
        ...
        <nav className="flex-1 px-3 py-6 space-y-1 overflow-y-auto font-geist-sans">
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                onClick={() => setSidebarOpen(false)}
                className={`flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition-none group ${
                  active
                    ? "bg-primary text-primary-foreground shadow-md shadow-primary/20"
                    : "text-sidebar-foreground/70 hover:bg-sidebar-accent hover:text-sidebar-foreground"
                }`}
              >
                <Icon className={`h-4 w-4 transition-none ${active ? "text-primary-foreground" : "text-sidebar-foreground/50 group-hover:text-sidebar-foreground"}`} />
                {item.label}
              </Link>
            );
          })}
          ...
          <div className="px-3 pb-2 space-y-1">
            ...
            >
              <BookOpen className={`h-4 w-4 transition-none ${isActive({ href: "/dashboard/docs" }) ? "text-primary-foreground" : "text-sidebar-foreground/50 group-hover:text-sidebar-foreground"}`} />
              Help & Docs
            </Link>
          </div>
          ...
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium text-destructive hover:bg-destructive/10 transition-none"
          >
        ...
        <header className="bg-background/80 backdrop-blur-md border-b border-border px-4 md:px-6 h-16 flex items-center justify-between gap-4 flex-shrink-0 sticky top-0 z-30">
          <div className="flex items-center gap-4">
            <button
              ...
              className="md:hidden p-2 rounded-lg text-muted-foreground hover:bg-muted transition-none"
            >
